...
For each metric, there are two and perhaps three quantities that can be usefully compared:
 ...
 - estimated :: the value of the metric for the given time, measured from the actually colleted visits and visit parameters in the same way they were measured against the baseline simulation.
...
*** TODO Total, mean, median, min, max, and quantiles of numbers of visits (specified in table 23 of [[http://ls.st/lpm-17][LPM-17]])
*** TODO Numbers of science visits by band
*** TODO Numbers of science visits by R.A.
*** TODO Numbers of science visits by program (WFD, DDF, ToO, etc.)
*** TODO Total, mean, median, min, max, and quantiles of depth or $t_{eff}$
*** TODO Total accumulated t_eff by band
*** TODO Total accumulated t_eff by R.A.
...
#+end_comment
*** TODO List relevant metrics from PSTN-51
...
The list of metrics tracked this way should be the same as that in section [[*Scalar Metrics]].
...
Examples of such diagnostics include:
 ...
 - DDF cadence plot :: A figure which shows the times series of sets of visits in each band on each DDF, with the current time since last occurrance of each DDF/band combination. Combining simple markers here with indicators of achieved limiting magnitude and best possible estimated limiting magintude for the next week would also be useful.
...
Examples of modeled characteristics include:
 - slew time :: as a function of change in horizon coordinates
 - filter change time ::
 - shutter and readout time ::
 - total overhead between successive exposures :: 
 - sky brightness :: as a function of airmass, sun and moon location and phase, and other factors
 - atmospheric seeing :: as measured by the DIMM
 - final delivered PSF width :: combines atmospheric seeing, the turbulance outer scale, and instrumental contributions
 - extinction and lost time due to clouds ::
 - time lost due to engineering activities and equipment failures ::
 - achieved $5-\sigma$ depth :: 
...
*** TODO Time use "hourglass" plots
*** TODO Current depth map
*** TODO Best FWHM map
