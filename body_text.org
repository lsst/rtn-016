...
This list of processes indicates some elements of computing infrastructure that will be needed:
 ...
 - simulation archive :: An archive to store the visit databases, and store metadata on each. Metadata will include things like the date at which collected visits end and simulated ones begin, weather (seeing and cloud) databases used, =opsim= version used, and instrument parameters used.
 - metric archive :: An archive to store the MAF results, and store metadata on each. Metadata will need to include the visit database used, subsets of data within that simulation database (particularly ranges of dates), and versions of MAF used.
