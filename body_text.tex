...
\section{Use Cases}
\label{sec:orgaa4a4b8}
\subsection{Pre-night review}
\label{sec:org1cd5a24}
...
\begin{itemize}
\item a cadence plot, showing the nights on which each DDF field was observed, and the depths of the images obtained. Figure \ref{fig:org60b52e8} shows a sample cadence plot.
\item a time use hourglass plot, showing the nights on which each DDF field was observed, and the time it was observed relative to transit. Figure \ref{fig:org457d1b4} shows an example time use hourglass plot.
...
\label{sec:org9bab63a}
\subsection{Survey state descriptions}
\label{sec:org649cbf2}
...
\item[{deep drilling field (DDF) cadence plots}] A set of plots (one for each DDF) showing the nights on which each DDF has been observed in the current season, and marking the time since it was last observed in each filter. Such plots may also encode the depth of exposures in each band on each night. Figure \ref{fig:org60b52e8} shows a sample cadence plot made using simulated data.
...
\caption{\label{fig:orga7262eb}Example depth map from MAF, for the state of the survey at the conclusion of the 10 year baseline simulation.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[height=0.4\textheight]{./figures/baseline_nexp2_v1_7_10yrs_fO_All_visits_HEAL_FO.pdf}
\caption{\label{fig:orgdb9e900}Example depth-area plot from MAF, for the state of the survey at the conclusion of the 10 year baseline simulation.}
\end{figure}
...
\caption{\label{fig:org91821fb}Distributions of the number of visits in the best 18000 square degrees by year, for the the 1.7 2 visit baseline. The central line shows the median over the footprint, the box the first and third quartiles, and the whiskers the 5\% and 95\% quantiles. A corresponding figure used for tracking progress should also include indicators of reference distributions, for example by using a shaded background.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figures/progress_by_band.png}
\caption{\label{fig:org50490fe}Numbers of visits in each filter as a function of date, with the moon phase along the bottom. A corresponding figure used for tracking purposes should also include indicators of reference distributions, for example by using dashed lines or a shaded background.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figures/progress_by_quadrant.png}
\caption{\label{fig:orgcb3b46d}Numbers of visits in each filter as a function of quarter of the sky. A corresponding figure used for tracking purposes should also include indicators of reference distributions, for example by using dashed lines or a shaded background.}
\end{figure}

\subsection{Projected scalar metrics for the final survey as a function of time}
\label{sec:org68c889e}
...
The list of metrics tracked this way should be the same as that in section \ref{sec:org475141e}.

\subsection{Nightly scheduler behavior diagnostics}
\label{sec:org1ed016c}
...
\item[{DDF cadence plots}] The DDF cadence plots described in section \ref{sec:org649cbf2} will also be important for understanding whether a DDF should be (or should have been) observed on a given night.
...
Figure \ref{fig:orgb99d787} shows an example on such a diagram.
...
Comparing these hourglass plots to the time use hourglass plot (figure \ref{fig:org457d1b4}), it can be seen that these are DDF fields, and that they are being observed well before transit.
...
\label{sec:orgaf1e929}
\subsection{Introduction}
\label{sec:org546a268}
...
Under any plan, it will be worthwhile to automate the generation of the necessary scheduler simulations and diagnostics (listed in section \ref{sec:org1ed016c}) for review and inclusion in briefing for the observing specialists and a plan for the night.
...
\item Automatic creation of the diagnostics listed in section \ref{sec:org1ed016c}, based on the simulations.
...
\begin{itemize}
\item Automatic creation of scheduler simulations. The initial simulation for the night may be the same one as that described in section \ref{sec:org2f612e1}, but additional simulations throughout the night will also be required.
...
This report is a natural home for the nightly scheduler behavior diagnostics (described in section \ref{sec:org1ed016c}), when applied to actual (as opposed to simulated) scheduled nights.
Furthermore, some elements of the survey state description (section \ref{sec:org649cbf2}) will be of broad enough interest that updates to them may be usefully included after each night.

In addition to the diagnostics directly related to scheduling, several of the data quality indicators that will be reported in the night report (following LSE-61/DMS-REQ-0097) to monitor the health of other subsystems are close to those needed for validation of the scheduler's site and telescope model (section \ref{sec:org17f1890}). If these elements are produced with the needs of the scheduler scientists in mind, these same plots may fill both needs.

So, to support scheduler and survey progress monitoring, the night report should include:
\begin{itemize}
\item Comparisons of system characteristics (slew time, filter change time, depth, sky brightness, etc.) with models used by the scheduler simulator (some subset of the diagnostics listed in section \ref{sec:org17f1890}).
\item Nightly scheduler behavior diagnostics (most or all diagnostics listed in section \ref{sec:org1ed016c}).
\item Updated diagnostics for the survey state (a selection of the diagnostics listed in section \ref{sec:org649cbf2}).
\end{itemize}
...
detailed monitoring will require all diagnostics listed in section \ref{sec:org9bab63a}.
...
All of these reports may draw from any of the report elements described in section \ref{sec:org9bab63a}, but it is unlikely that any single report will require every element.
...
\section{Workflows}
\label{sec:org4e40b69}
\begin{figure}[htbp]
\centering
\includegraphics[height=0.9\textheight]{./figures/reportdfd.png}
\caption{\label{fig:org4e2b180}A data flow diagram for producing an LSST observing progress report.}
\end{figure}

The workflow for producing reports will have several steps, each reading some sets of data, and producting others. Figure\textasciitilde{}\ref{fig:org4e2b180} shows the data flow for the production of a progress report, with the different processes labeled by simple letters:
...
\begin{description}
\item[{\texttt{MAF}}] \texttt{MAF} itself will be needed to calculate and plot metrics. In addition to the metrics and plots currently available, several additional ones will need to be developed.
\item[{\texttt{opsim}}] \texttt{opsim} will be needed to create visits sets both for the baseline, and ranges of possible survey futures.
