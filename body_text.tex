...
\label{sec:org6b572bc}
\subsection{Survey state descriptions}
\label{sec:org7d6427e}
...
\item[{deep drilling field (DDF) cadence plots}] A set of plots (one for each DDF) showing the nights on which each DDF has been observed in the current season, and marking the time since it was last observed in each filter. Such plots may also encode the depth of exposures in each band on each night. Figure \ref{fig:org5b38321} shows an analogous plot for the SN fields for the Dark Energy Survey (DES). A figure suitable for the LSST DDF survey will be similar.
...
\caption{\label{fig:orgee75974}Example depth map from MAF, for the state of the survey at the conclusion of the 10 year baseline simulation.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[height=0.4\textheight]{./figures/baseline_nexp2_v1_7_10yrs_fO_All_visits_HEAL_FO.pdf}
\caption{\label{fig:org47c83fb}Example depth-area plot from MAF, for the state of the survey at the conclusion of the 10 year baseline simulation.}
\end{figure}
...
\caption{\label{fig:org7e536ab}Example filter use hourglass plot from MAF, for the state of the survey at the conclusion of the 10 year baseline simulation. The black line along the bottom shows the lunar phase. The red and blue lines show nautical and civil twilight.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[height=0.4\textheight]{./figures/snhistory.pdf}
\caption{\label{fig:org5b38321}An example DES SN cadence plot, analogous to an LSST DDF cadence plot. Each horizontal element in the plot represents a different class of sequence of visits on a specific field. Colored vertical lines within each element represent exposures within those sequences in different filters. Large numbers on the right of the plot indicate number of nights since a sequence of that class was completed for that field.}
\end{figure}

\subsection{Time series progression of scalar survey metrics}
\label{sec:org9fca146}
...
\caption{\label{fig:orgebce062}Distributions of the number of visits in the best 18000 square degrees by year, for the the 1.7 2 visit baseline.}
\end{figure}

\subsection{Predicted scalar metrics for the final survey as a function of time}
\label{sec:orgbb17f97}
...
The list of metrics tracked this way should be the same as that in section \ref{sec:org9fca146}.

\subsection{Nightly scheduler behaviour diagnostics}
\label{sec:org4b910cc}
...
\item[{DDF cadence plots}] The DDF cadence plots described in section \ref{sec:org7d6427e} will also be important for understanding whether a DDF should be (or should have been) observed on a given night.
...
\label{sec:org7fd9297}
\subsection{Introduction}
\label{sec:org42a9196}
...
Under any plan it would be worthwhile to automate the generation of the necessary scheduler simulations and diagnostics (listed in section \ref{sec:org4b910cc}) for review and inclusion in briefing for the observing specialists and a plan for the night.
...
\item Automatic creation of the diagnostics listed in section \ref{sec:org4b910cc}, based on the simulations.
...
\begin{itemize}
\item Automatic creation of scheduler simulations. The initial simulation for the night may be the same one as that described in section \ref{sec:orgff57f12}, but additional simulations throughout the night will also be required.
...
This report is a natural home for the nightly scheduler behaviour diagnostics (described in section \ref{sec:org4b910cc}), when applied to actual (as opposed to simulated) scheduled nights.
Furthermore, some elements of the survey state description (section \ref{sec:org7d6427e}) will be of broad enough interest that updates to them may be usefully included after each night.

In addition to the diagnostics directly related to scheduling, several of the data quality indicators that will be reported in the night report (following LSE-61/DMS-REQ-0097) to monitor the health of other subsystems are close to those needed for validation of the scheduler's site and telescope model (section \ref{sec:orgffee80a}). If these elements are produced with the needs of the scheduler scientists in mind, these same plots may fill both needs.

So, to support scheduler and survey progress monitoring, the night report should include:
\begin{itemize}
\item Comparisons of system characteristics (slew time, filter change time, depth, sky brightness, etc.) with models used by the scheduler simulator (some subset of the diagnostics listed in section \ref{sec:orgffee80a}).
\item Nightly scheduler behaviour diagnostics (most or all diagnostics listed in section \ref{sec:org4b910cc}).
\item Updated diagnostics for the survey state (a selection of the diagnostics listed in section \ref{sec:org7d6427e}).
\end{itemize}

\subsection{Tools for performance evaluation and analysis}
\label{sec:org065549b}
...
detailed monitoring will require all diagnostics listed in section \ref{sec:org6b572bc}.
...
All of these reports may draw from any of the report elements descibed in section \ref{sec:org6b572bc}, but it is unlikely that any single report will require every element.
